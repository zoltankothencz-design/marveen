...
- Annak a parent process-e az AGENT-é (`ps -o command -p <ppid>` tartalmazza az agent nevét vagy a `--channels plugin:slack-channel` flaget)
- The operator DM-je a Slack agent tmux pane-jén megjelenik `<channel source="slack" ...>` tag-ben
